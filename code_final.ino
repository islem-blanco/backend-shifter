...
#define DHT_INTERVAL      30000   // 30 secondes
#define MPU_INTERVAL       3000   // 3 secondes
#define PING_INTERVAL     60000   // 60 secondes
#define SLEEP_INTERVAL   120000   // 2 minutes entre cycles
#define ANOMALY_STAY_MS  180000   // Rester actif 3 min après anomalie
...
bool reopenMQTT() {
  ...
  if (!sendAT(cmd, "CONNECT", 20000)) {
    Serial.println("[MQTT] CIPSTART échoué");
    mqttConnecte = false;
    return false;
  }
  delay(500);
  if (!mqttConnect()) {
    Serial.println("[MQTT] CONNACK échoué");
    mqttConnecte = false;
    return false;
  }
  mqttConnecte = true;
...
void mqttPublish(const char* topic, const String& payload) {
  if (!mqttConnecte) {
    Serial.println("[MQTT] ⚠️ Pas connecté");
    return;
  }
...
void processNMEA_RMC(const String& sentence) {
  // Format: $GPRMC,hhmmss.ss,A,ddmm.mmmm,N/S,dddmm.mmmm,E/W,speed,course,ddmmyy,...
  // Fields: 0=GPRMC, 1=heure, 2=status, 3=lat, 4=lat_dir, 5=lon, 6=lon_dir, 7=speed, 8=course, 9=date
  ...
  String statut  = champs[2];
  if (statut != "A") {
    Serial.println("⏳ GPS pas actif (status=" + statut + ")");
    return;
  }

  String latNmea = champs[3];
  String latDir  = champs[4];
  String lonNmea = champs[5];
  String lonDir  = champs[6];
  String vitesse = champs[7];
  String date    = champs[9];
  String heure   = champs[1];

  if (latNmea.length() == 0 || lonNmea.length() == 0) {
    Serial.println("⏳ Coordonnées vides");
    return;
  }

  float lat = nmeaToDegrees(latNmea);
  float lon = nmeaToDegrees(lonNmea);
  if (latDir == "S") lat = -lat;
  if (lonDir == "W") lon = -lon;

  float kmh = vitesse.toFloat() * 1.852;
  ...
  if (!gpsFixed) {
    ...
    Serial.println("║  DHT/MPU en route...                ║");
    Serial.println("╚════════════════════════════════════╝\n");
  }

  // Reverse geocoding toutes les 60s
  if (derniereVille == "..." || millis() - dernierAppel > 60000) {
    Serial.println("[GEO] Recherche ville...");
    derniereVille = getVille(lat, lon);
    dernierAppel  = millis();
  }

  String h     = heure.substring(0, 2);
  String m     = heure.substring(2, 4);
  String s     = heure.substring(4, 6);
  String jour  = date.substring(0, 2);
  String mois  = date.substring(2, 4);
  String annee = "20" + date.substring(4, 6);
...
void publishDHT() {
  ...
  if (!mqttConnecte) {
    if (!reopenMQTT()) {
      Serial.println("[MQTT] ❌ Reconnexion impossible");
      return;
    }
  }
  ...
  // Ajouter package_id si kit actif
  if (kitActif) {
    doc["package_id"] = packageId;
  }
...
void processMqttPacket() {
 ...
 if (payload.indexOf("\"deliver\"") != -1) {
    ...
    mqttPublish(MQTT_TOPIC_STATUS, String(msg));
    delay(500);
    // Deep sleep permanent — batterie préservée après livraison
...
void readSerial1() {
  while (Serial1.available()) {
    ...
    // ── Traitement MQTT entrant ──────────────────────────
    if ((uint8_t)c == 0x30 && !gpsBuffer.length()) {  // MQTT PUBLISH byte
      mqttInMode = true;
      mqttInLen  = 0;
...
void setup() {
  ...
  Serial.println("[MPU] DLPF 42 Hz activé");
  ...
  // ── GPS allumage + NMEA Stream ──────────────────────────
  Serial.println("[GPS] Allumage...");
  Serial1.println("AT+CGPSPWR=1");
  delay(1000);
  while (Serial1.available()) Serial1.read();

  Serial.println("[GPS] Activation du flux NMEA...");
  sendAT("AT+CGPSOUT=255", "OK", 3000);
  flushSerial1();
  Serial.println("[GPS] ✓ NMEA streaming actif");

  // ── Arrêt NMEA avant GPRS ──────────────────────────────
  gpsStopStream();
  ...
  Serial.println("\n╔════════════════════════════════════════════╗");
  Serial.println("║  SAHRA v3.1 — NMEA Streaming — Prêt !    ║");
  Serial.println("║  GPS: flux NMEA continu                   ║");
  Serial.println("║  DHT/MPU: démarrage après GPS fix         ║");
  Serial.println("╚════════════════════════════════════════════╝\n");
}
...
// ════════════════════════════════════════════════════════════
void loop() {
  // ── Lire commandes MQTT + NMEA GPS ─────────────────────
  readSerial1();

  // ── DHT toutes les 15s — après fix GPS ──────────────────
  if (gpsFixed && millis() - dernierDHT > 15000) {
publishDHT();
    dernierDHT = millis();
  }

  // ── MPU toutes les 2s — après fix GPS ───────────────────
  if (gpsFixed && millis() - dernierSensor > 2000) {
    publishMPUIfAnomaly();
    dernierSensor = millis();
  }

  // ── PING MQTT toutes les 50s ────────────────────────────
  if (mqttConnecte && millis() - dernierPing > 50000) {
    mqttPing();
    dernierPing = millis();
  }
